...
int isFinish;
// victor
extern "C" __declspec(dllexport)void  stopRecover( )
{
    printf("stopRecover++\n");
    mStopRecovrey=1;
}
// victor
extern "C" __declspec(dllexport)int  getInformation(  char* buf,char* buf1,char* buf2,char* buf3,char* buf4  )
{
    strcpy(buf, mElapsed_time);
    strcpy(buf1, mEstimated_time);
    strcpy(buf2, mPercent);
    strcpy(buf3,mReadingsector);
    strcpy(buf4,mFileFound);
    return isFinish;
    //  return "getInformation";
}
// victor
extern "C" __declspec(dllexport) list_disk_t *hd_parse_dll(list_disk_t *list_disk, const int verbose, const int testdisk_mode)
{
    list_disk = NULL;
    list_disk = hd_parse(list_disk, verbose, testdisk_mode);
    //char s[] = "size: ";
    //sprintf(s, "%d %d", sizeof(partition_t), sizeof(efi_guid_t));
    ...
    for(list_disk_t *element_disk = list_disk;element_disk != NULL; element_disk = element_disk->next)
...
// victor
extern "C" __declspec(dllexport)int startRevocer(char path[], char condition[],char outfolder[])
{
    int i;
#ifdef SUDO_BIN
    int use_sudo=0;
    ...
    isFinish=0;
    memset(mElapsed_time, 0, sizeof(mElapsed_time));
    memset(mEstimated_time, 0, sizeof(mEstimated_time));
    memset(mPercent, 0, sizeof(mPercent));
    memset(mReadingsector, 0, sizeof(mReadingsector));
    memset(mFileFound, 0, sizeof(mFileFound));
    ...
    //printf("Transcend RevocerRX v4.0\n");
    int argc = 4;
    char argv[4][1024]= {"","","\\\\.\\PhysicalDrive2", "fileopt,everything,enable,freespace,search"};
    // char argv[4][80];
    strncpy(argv[2], path, 80);
    strncpy(argv[3], condition, 1024);
    mStopRecovrey=0;
    //https://www.cgsecurity.org/wiki/Scripted_run
    sprintf(outputpath,"%s",outfolder);
    for(i=1;i<argc;i++)
    {
        if(i+2>=argc)
        {
