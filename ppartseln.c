...
void menu_photorec(struct ph_param *params, struct ph_options *options, alloc_data_t*list_search_space)
{
  ...
  if(params->cmd_run!=NULL)
  {
   ...
   //  snprintf(res,"%s",outputpath);

#endif
    if(outputpath!=NULL)
	{
      params->recup_dir=(char *)MALLOC(strlen(outputpath)+1+strlen(DEFAULT_RECUP_DIR)+1);
      strcpy(params->recup_dir,outputpath);
