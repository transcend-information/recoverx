...
void photorec_info(  const file_stat_t *file_stats)
{
    ...
    for(i=0; i<9 && i<nbr && new_file_stats[i].recovered>0; i++)
    {
        ...
        wprintw(window, "%s: %u recovered\n",
                (new_file_stats[i].file_hint->extension!=NULL?
                    new_file_stats[i].file_hint->extension:""),
                new_file_stats[i].recovered);
        printf("%s: %u recovered\n",
               (new_file_stats[i].file_hint->extension!=NULL?
                    new_file_stats[i].file_hint->extension:""),
               new_file_stats[i].recovered);*/
        char tmp[30];
        sprintf(tmp,"%s:%u,", (new_file_stats[i].file_hint->extension!=NULL?
                    new_file_stats[i].file_hint->extension:""),
                new_file_stats[i].recovered);
        strcat(mFileFound, tmp);
    }
    for(;i<nbr && new_file_stats[i].recovered>0;i++)
        others+=new_file_stats[i].recovered;
    if(others>0)
    {
        ...
        // wclrtoeol(window);
        //wprintw(window, "others: %u recovered\n", others);
       // printf("others: %u recovered\n", others);
        char tmp[20];
        sprintf(tmp,"others: %u",others);
        strcat(mFileFound, tmp);
    }
    free(new_file_stats);
}
// victor
pstatus_t set_photorec_progressbar ( const unsigned int pass, const struct ph_param *params, const uint64_t offset, const time_t current_time)
{
    const partition_t *partition=params->partition;
    const unsigned int sector_size=params->disk->sector_size;
    if(params->status!=STATUS_FIND_OFFSET)
    {
        ...
        //  wclrtoeol(window);
        //  wprintw(window,"Destination %s", params->recup_dir);
        //  printf("Destination %s", params->recup_dir);
    }
    // wmove(window,10,0);
    //wclrtoeol(window);
    if(params->status==STATUS_EXT2_ON_BF || params->status==STATUS_EXT2_OFF_BF)
    {
        ...
        //     (unsigned long)((offset-partition->part_offset)/sector_size),
        // pass);
        printf("Bruteforce %10lu sectors remaining (test %u), ",
               (unsigned long)((offset-partition->part_offset)/sector_size),
               pass);
    }
    else
    ...
    (unsigned long long)((offset-partition->part_offset)/sector_size),
    (unsigned long long)(partition->part_size/sector_size));
        printf("Pass %u - Reading sector %10llu/%llu, ",
               pass,
               (unsigned long long)((offset-partition->part_offset)/sector_size),
               (unsigned long long)(partition->part_size/sector_size));*/
        sprintf(mReadingsector,"%10llu/%llu",  (unsigned long long)((offset-partition->part_offset)/sector_size),
                (unsigned long long)(partition->part_size/sector_size));
    }
    if(params->status==STATUS_FIND_OFFSET)
        //   wprintw(window,"%u/10 headers found\n", params->file_nbr);
        printf("%u/10 headers found\n", params->file_nbr);
    else
    {
        // wprintw(window,"%u files found\n", params->file_nbr);
    ...
    if(current_time > params->real_start_time)
    {
        ...
        /* wprintw(window,"Elapsed time %uh%02um%02us",
                ...
                (unsigned)(elapsed_time/60%60),
                (unsigned)(elapsed_time%60));
        printf("Elapsed time %uh%02um%02us",
               (unsigned)(elapsed_time/60/60),
               (unsigned)(elapsed_time/60%60),
               (unsigned)(elapsed_time%60));*/
        sprintf(mElapsed_time,"%u:%02u:%02u",(unsigned)(elapsed_time/60/60),
                (unsigned)(elapsed_time/60%60),
                (unsigned)(elapsed_time%60));
        /* sprintf(ElapsedTime,"%uh%02um%02us",(unsigned)(elapsed_time/60/60),
                (unsigned)(elapsed_time/60%60),
                (unsigned)(elapsed_time%60));*/
        if(offset > partition->part_offset && params->status!=STATUS_EXT2_ON_BF && params->status!=STATUS_EXT2_OFF_BF)
        {
            ...
            /* wprintw(window," - Estimated time to completion %uh%02um%02u\n",
                    ...
                    (unsigned)((eta/60)%60),
                    (unsigned)(eta%60));
            printf(" - Estimated time to completion %uh%02um%02u\n",
                   (unsigned)(eta/3600),
                   (unsigned)((eta/60)%60),
                   (unsigned)(eta%60));*/
            sprintf(mEstimated_time,"%u%02u%02u",     (unsigned)(eta/3600),
                    (unsigned)((eta/60)%60),
                    (unsigned)(eta%60));

            if(params->offset>0)
            {
                int percent = (params->offset-partition->part_offset)*100/ partition->part_size;
                 printf(" - complete percent %d %%\n",percent);
                sprintf(mPercent,"%d",percent);
            }
